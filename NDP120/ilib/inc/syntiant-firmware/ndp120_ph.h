...
struct ndp120_ph_params_collection_s {
    struct ndp120_ph_params_s sph_params[MAX_NNETWORKS];
    uint32_t num_params;
};
 ...
 * @param ph_state Posterior handler state
 */
void ndp_ph_process_frame(struct ndp120_ph_state_s *ph_state,
 ...
 * @param ph_state Posterior handler state
 */
int ndp120_single_counter_frame_processor(struct ndp120_ph_state_s *ph_state,
                                           struct ndp120_result_s *result, uint32_t tankptr);
 ...
 */
int ndp120_multi_counter_frame_processor(struct ndp120_ph_state_s *ph_state,
                                          struct ndp120_result_s *result, uint32_t tankptr);
...
struct ndp120_ph_state_params_s *
ndp120_ph_state_params_get(struct ndp120_ph_params_s *ph_params,
        uint32_t state_num);

void ndp_result_compute_softmax(struct ndp120_result_s *result,
        uint16_t scale_factor, uint8_t activation_type);
