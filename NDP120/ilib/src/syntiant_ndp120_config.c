...
#define PLL_LOCK_WAIT_IN_MSEC   3000
...
static int toggle_pdmctl_update_if_enabled(struct syntiant_ndp_device_s *ndp, uint32_t interface) {
    uint32_t pdmctl_data;
    int is_enabled;
    int s = NDP_MB_ERROR_NONE;
    uint32_t adx = NDP120_DSP_CONFIG_PDMCTL(interface);

    s = ndp_mcu_read(adx, &pdmctl_data);
    if(s) goto error;

    is_enabled = NDP120_DSP_CONFIG_PDMCTL_ENABLE_EXTRACT(pdmctl_data);
    if (!is_enabled) {
        uint32_t i2sctl_data;
        s = ndp_mcu_read(NDP120_DSP_CONFIG_I2SCTL(interface), &i2sctl_data);
        if (s) goto error;
        is_enabled = NDP120_DSP_CONFIG_I2SCTL_ENABLE_EXTRACT(i2sctl_data);
    }
    
    if (!is_enabled) goto error;
    ...
    struct syntiant_ndp_device_s *ndp, syntiant_ndp120_config_clk_pll_t *config)
{
    ...
    if (config->set & NDP120_CONFIG_SET_CLK_PLL_PRESET) {
        ...
        XNO_Printf("NDP: waiting for pll to be locked");
        osDelay(5000);//give some delay
        if (!config->no_switch) {
...
int syntiant_ndp120_config_decimation(struct syntiant_ndp_device_s *ndp, syntiant_ndp120_config_decimation_t *config)
{
    ...
    if (old_data != data) {
        ...
        if (s) goto error;
        s = toggle_pdmctl_update_if_enabled(ndp, config->mic >> 1);
        if (s) goto error;
    }
...
int syntiant_ndp120_config_farrow(struct syntiant_ndp_device_s *ndp, syntiant_ndp120_config_farrow_t *config){
    ...
    if (old_data != data) {
        ...
        if (s) goto error;
        s = toggle_pdmctl_update_if_enabled(ndp, config->interface);
        if (s) goto error;
    }
