...
#include <ndp120_dsp_sample.h>
#include <ndp120_dsp_mailbox.h>
#include <ndp120_dsp_memory.h>
...
#include <ndp120_result.h>
#include <ndp120_dnn_regs.h>
#ifndef __KERNEL__
//#include <unistd.h>
//#include <sys/time.h>
#endif
...
static int syntiant_ndp120_write_softmax_config(
        ...
        uint32_t *data, uint32_t size, int write)
{
    int s = 0;
    syntiant_ndp120_device_t *ndp120 = &ndp->d.ndp120;
    uint32_t adx = ndp120->dsp_fw_state_addr + (uint32_t) offsetof(ndp120_dsp_fw_base_t,
            dnn_params_power_cfg);
    if (write) {
        s = ndp_mcu_write_block(adx, data, size);
    } else {
        s = ndp_mcu_read_block(adx, data, size);
                                    ...
                                    ndp120_dsp_config_t *config)
{
    ...
    /* ping DSP to re-allocate memory for sample tanks */
    s = syntiant_ndp120_do_mailbox_req(ndp, NDP120_DSP_MB_H2D_PING,
                                       NULL);
error:
    return s;
}
...
{
    int s = SYNTIANT_NDP_ERROR_NONE;
    uint32_t adx, avail_mem, audio_sample_size_bytes, max_winstep_us;
    ...
    audio_sample_size_bytes = (uint32_t) (max_winstep_us * NDP120_AUDIO_SAMPLE_RATE
                                          * NDP120_AUDIO_SAMPLES_PER_WORD / 1000000);
    ...
    /* Find larger of HEAP and DNN memory, minus amount of memory needed
       for func tank + overhead. For now, func_samp_size_bytes & func_samp_cap
       are not user configurable */
    avail_mem = (mem_state.mem_heap_total - mem_state.algo_mem_use_heap >=
                mem_state.mem_dnn_data_total - mem_state.algo_mem_use_dnn_data)
                ? mem_state.mem_heap_total - mem_state.algo_mem_use_heap :
                mem_state.mem_dnn_data_total - mem_state.algo_mem_use_dnn_data;
    avail_mem -= (uint32_t) NDP120_DSP_MEM_ALLOC_TRUE_SIZE
                    (cfg.func_samp_size_bytes * cfg.func_samp_cap);

    /* For remaining memory: round aud_samp_cap down to integer */
    if (config->get) {
        /* get the current configuration */
        config->sampletank_msec = cfg.aud_samp_cap * max_winstep_us / 1000;
    } else if (config->sampletank_msec) {
        cfg.aud_samp_cap = config->sampletank_msec * 1000 / max_winstep_us;
        if (avail_mem >= NDP120_DSP_MEM_ALLOC_TRUE_SIZE(
                          cfg.aud_samp_cap * audio_sample_size_bytes) +
                         NDP120_DSP_MEM_ALLOC_TRUE_SIZE(
                          cfg.aud_samp_cap *
                          sizeof(ndp120_dsp_audio_sample_annotation_t))) {
            cfg.aud_samp_size_bytes = audio_sample_size_bytes;

            /* need to re-calculate recording duration due to
               possible rounding from above */
            config->sampletank_msec = cfg.aud_samp_cap * max_winstep_us
                                         / 1000;
            syntiant_ndp120_write_sample_config(ndp, &cfg);
        } else {
            s = SYNTIANT_NDP_ERROR_NOMEM;
        }
    } else if (!config->sampletank_msec) {
        cfg.aud_samp_cap = (uint32_t) ((avail_mem -
                            NDP120_DSP_MEM_ALLOC_TRUE_SIZE(0)) /
...
syntiant_ndp120_pkg_parse_nn_softmax (struct syntiant_ndp_device_s *ndp) {
    ...
    if (pstate->mode == PACKAGE_MODE_VALUE_START) {
        ...
        s = syntiant_ndp120_write_softmax_config(ndp, &pstate->data.softmax);
        if (s) goto error;
    }
...
syntiant_ndp120_pkg_parse_dnn_power_config_v1(struct syntiant_ndp_device_s *ndp)
{
    ...
    if (pstate->mode == PACKAGE_MODE_VALUE_START) {
        ...
        s = syntiant_ndp120_read_write_dnn_power_cfg(ndp, (uint32_t *) &pstate->data.dnn_power,
                             ...
                             int from, uint8_t *data, unsigned int *lenp)
{
    ...
    if (fw_buffers.sample_reset_flag) {
        last_ptr = 0;
        adx = ndp120->dsp_fw_state_addr + (uint32_t)offsetof(ndp120_dsp_fw_base_t, buffers.sample_reset_flag);
    ...
    wanted_chunk_count = *lenp / effective_sample_size;
    *lenp = wanted_chunk_count * effective_sample_size;
    ...
    chunk_count = MIN(wanted_chunk_count, chunk_count);
    *lenp  = chunk_count * effective_sample_size;
    ...
    switch (from) {
        case SYNTIANT_NDP_EXTRACT_FROM_MATCH: {
            ...
            if (data == NULL) {  
                // Set pre-roll based on wanted_chunk_count
                read_start_ptr -=  wanted_chunk_count * sample_size;
                if(read_start_ptr < buf_start){
                    read_start_ptr += buf_size_bytes;
                }
            }
            break;
        }
        ...
        case SYNTIANT_NDP_EXTRACT_FROM_NEWEST:
            if (data == NULL) {
                read_start_ptr = prod_ptr - wanted_chunk_count * sample_size;
            } else {
        ...
        struct syntiant_ndp_device_s *ndp, uint32_t network_id, uint8_t *act)
{
    ...
    case NDP120_DNN_ISA_COMP0_ACTIVATION_SIGMOID1:
        *act = sizeof(((struct ndp120_result_s *)0)->raw_strengths.u8[0]);
        break;
    ...
    case NDP120_DNN_ISA_COMP0_ACTIVATION_TANH1:
        *act = sizeof(((struct ndp120_result_s *)0)->raw_strengths.s8[0]);
        break;
    case NDP120_DNN_ISA_COMP0_ACTIVATION_LINEAR_16:
        *act = sizeof(((struct ndp120_result_s *)0)->raw_strengths.s16[0]);
        break;
    default:
...

{
    ...
    if (type == SYNTIANT_NDP_STRENGTH_RAW) {
        ...
        if (s) goto error;
        len = len * act_size;
        if (len > NDP120_RESULT_NUM_CLASSES * (int) sizeof(uint16_t) ||
                len % 4 != 0) {
            s = SYNTIANT_NDP_ERROR_ARG;
            goto error;
        }
    } else if (type == SYNTIANT_NDP_STRENGTH_SOFTMAX) {
        if (len > NDP120_RESULT_NUM_CLASSES * (int) sizeof(uint32_t) ||
                len % 4 != 0) {
            s = SYNTIANT_NDP_ERROR_ARG;
            goto error;
        }
    ...
    if (type == SYNTIANT_NDP_STRENGTH_RAW) {
        ...
        if (addr) {
            syntiant_ndp120_get_result_offset(&addr, network_id);
            addr += (uint32_t) offsetof(struct ndp120_result_s,
                    softmax_strengths);
            s = syntiant_ndp120_read_block(ndp, 1, addr, strengths, len);
        } else {
        /* FAIL instead of UNINIT to match 10x ilib code */
...
static int
syntiant_ndp120_write_softmax_config(struct syntiant_ndp_device_s *ndp,
        syntiant_softmax_t *softmax)
{
    ...
    addr += (unsigned int) sizeof(struct ndp120_result_softmax_smoother_s) * softmax->nn_num +
    ...
    DEBUG_PRINTF("writing softmax scale factor %u to 0x%x for nn:%d\n",
            softmax->quantized_sf, addr, softmax->nn_num);
    return syntiant_ndp120_write(ndp, 1, addr, (uint32_t) softmax->quantized_sf);
}
